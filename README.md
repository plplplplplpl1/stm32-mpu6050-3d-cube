...
| English | 中文 |
|---------|------|
| **Software SPI** 驱动 OLED（1.3寸 7脚模块）+ **Software I2C** 驱动 MPU6050 | **软件 SPI**（PB5/PB6/PB7/PB8/PB9）驱动 OLED + **软件 I2C**（PB10/PB11）驱动 MPU6050 |
...
| Display / 显示屏 | 1.3" OLED 128×64 (7-pin SPI) |
| Buttons / 按键 | 4x (KEY1, KEY2, KEY3-PA6, KEY4-PA4) |

## Pin Connections / 引脚连接

| STM32 | MPU6050 | OLED (SPI) | Encoder |
|-------|---------|------------|---------|
| PB10 | SCL | - | - |
| PB11 | SDA | - | - |
| PB5  | -   | CLK (SCK) | - |
...
| **OLED** | SSD1306 driver via **Software SPI** (PB5/PB6/PB7/PB8/PB9) | SSD1306 驱动，**软件 SPI**（PB5=SCK, PB6=MOSI, PB7=RES, PB8=DC, PB9=CS） |
...
### 2026-06-19 — SPI OLED 适配 + 滚轮菜单重构

- **硬件迁移**：0.96" I2C OLED → 1.3" 7 脚 SPI OLED（SH1106 芯片，SSD1306 兼容指令集）
  - 引脚：PB5=SCK, PB6=MOSI, PB7=RES, PB8=DC, PB9=CS
  - 软件 SPI 位带操作 ~5MHz，CS 常低逐页刷新
  - SH1106 列偏移 +2 消除右边缘鬼影（132 列内部 RAM，可视区从列 2 开始）
- **滚轮菜单系统重写**（Menu.c 260 行纯 C）：
  - 三项可见：选中项居中 20px 高亮反色条，上下项缩进显示
  - 像素精度文字渲染（16bit 拼接消除屏外裁剪间隙）
  - UNLINEAR 指数衰减动画：`current += (target - current) >> 3`，~250ms 平滑过渡
  - 高亮条固定居中，三项整体滑动穿过高亮区（老虎机效果）
  - 编码器循环切换，长按返回，短按确认
- **菜单项调整**：动画 → 动画，OLED → OLED 检测（6 张测试图切换），计步器 → 雷达（12x12 宋体字模）
- **OLED 检测功能**：白屏/黑屏/竖线/棋盘/仅 127 列亮/除 127 外亮 — 编码器切换测试图，硬件诊断用
- **子菜单滚轮化**：3D/2D/BACK 三项滚轮，统一动画逻辑
- **删除冗余**：去掉标题栏、滚动条、网点暗色效果，模块化 `SB`/`Inv`/`D1`/`PC` 组件
- **兼容性修复**：SH1106 列偏移 +2、负 Y 裁剪保护、页越界保护
